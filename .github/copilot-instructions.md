...
- **`src/context/RaceContext.js`** - Central Redux-style reducer for race state
  - Actions: `SET_STATUS`, `UPDATE_NODE`, `ADD_DETECTION`, `START_RACE`, `STOP_RACE`, `SELECT_FALCON`
  - Persists `selectedFalcon` and `races` to AsyncStorage
  - Always dispatch with `{ type, payload }` pattern

- **`src/ble/BleProvider.js`** - Singleton BLE manager
  - Exports: `useBle()` hook → `{ connectedDevice, scanning, connect(), disconnect(), write(), startScan() }`
  ...
  - **Message Flow**: TX notification (base64) → decode → buffer → `processStream()` → dispatch by `msg.data.type`
  - **Never await device.cancelConnection()** - it's fire-and-forget to avoid hangs

### Data Layer

- **`db/database.js`** - Realm schemas (v6)
...
- **`src/utils/parser.js`** - Defensive message parsing
  - `parseIncoming(raw)` → `{ kind: 'json'|'text', data }` - never throws
  - `processStream(bufferRef)` → extracts complete JSON objects from accumulated buffer
...
- **`components/dashboard.js`** - Main operator control panel
  ...
  - Handles: device scanning, connection, initial `get_status` command
  - Uses: `useBle()` + `useRace()` hooks
...
- **`components/racecontrol.js`** - Race execution screen
...
- **`components/theme.js`** - Design system constants
  - Colors: `desertSand` (bg), `charcoal` (text), `cobaltBlue` (primary), `oasisGreen` (success), `terracotta` (danger)
  - Fonts: `Montserrat-Regular`, `Montserrat-Bold`, `Orbitron-Bold` (numbers)
  - Base styles: `screen`, `title`, `number`
...
- **Set `ts_received: Date.now()`** on all incoming messages
...
- **Message log limit**: Keep last 200 messages only (prevent memory bloat)
...
- **State actions**: SCREAMING_SNAKE_CASE (`SET_STATUS`, `ADD_DETECTION`)
...
**Fix**: Check `BleProvider.js` - all handlers wrapped in try/catch; verify you're not adding code outside try blocks

### Messages Not Updating UI

**Cause**: RaceContext not receiving dispatch  
**Fix**: Add console.log in `handleNotification` after `dispatch()` calls; verify message type matches switch cases
...
**Fix**: Verify `START_RACE` action dispatched; check `currentRace.startTime` is set; ensure timer interval is running
...
- **Create new screen** → Follow drawer pattern in `sidebar.js`, use `useBle()` + `useRace()` hooks
...
- **Fix connection issues** → Check `handleDisconnection()` in BleProvider, verify exponential backoff
- **Parse new message type** → Add case in `handleNotification()`, update RaceContext reducer
- **Debug BLE** → Add console.logs with emoji prefixes (📨, ✅, ❌) to match existing pattern

**Always reference `ARCHITECTURE.md` for detailed component interaction flows.**
