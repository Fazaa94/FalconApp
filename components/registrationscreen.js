...
const RegistrationScreen = () => {
    ...
    try {
      realm.write(() => {
        realm.create('FalconRegistration', {
          id: uuid.v4(),
          animalId: formData.name.replace(/\s+/g, '-').toUpperCase() + '-' + Date.now().toString().slice(-4),
          falconName: formData.name,
  ...
  const handleSelectFalcon = (falcon) => {
    ...
    Alert.alert('Success', `${falcon.falconName} selected`);
  };

  const filteredFalcons = falcons.filter((f) =>
    f.falconName.toLowerCase().includes(searchQuery.toLowerCase())
  );
            ...
            {falcons.length === 0 ? <Text style={styles.emptyText}>No falcons registered</Text> : (
              <FlatList scrollEnabled={false} data={falcons} keyExtractor={(item) => item.id} renderItem={({ item }) => (
                <View style={styles.falconCard}>
                  <Text style={styles.falconName}>{item.falconName}</Text>
              ...
              <>
                ...
                <FlatList scrollEnabled={false} data={filteredFalcons} keyExtractor={(item) => item.id} renderItem={({ item }) => (
                  <TouchableOpacity style={styles.selectCard} onPress={() => handleSelectFalcon(item)}>
                    <View>
                      <Text style={styles.selectCardName}>{item.falconName}</Text>
                ...
                {raceState.selectedFalcon && (
                  <View style={styles.banner}>
                    ...
                    <Text style={styles.bannerName}>{raceState.selectedFalcon.falconName}</Text>
                  </View>
                )}
              </>
            )}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: COLORS.desertSand },
  ...
  input: { backgroundColor: COLORS.surfaceBg, color: COLORS.charcoal, paddingHorizontal: 12, paddingVertical: 10, borderRadius: 8, fontSize: 14, marginBottom: 8 },
  ...
  searchInput: { backgroundColor: COLORS.cardBg, color: COLORS.charcoal, paddingHorizontal: 12, paddingVertical: 10, borderRadius: 8, fontSize: 14, marginBottom: 16, borderWidth: 1, borderColor: COLORS.surfaceBg },
